...
# this will allow data to smaller and app to perform better
t <- read_csv('ships_04112020/ships.csv')
t <- t %>% mutate(SHIP_ID = as.character(SHIP_ID))
ship_types <- unique(t$ship_type)
write_rds(ship_types, 'data/ship_types.rds')

ship_ids <- unique(t$SHIP_ID)

for (i in ship_ids){
  print(i)
  temp <- t %>% filter(SHIP_ID == i)
  write_rds(temp,paste('data/',i,'.rds',sep=''))
}
